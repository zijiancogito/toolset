...
def make_exp(cas_num):
    _rnd = random_int32()
    if _rnd%cas_num == 0:
        return 'var%s = %svar%s' %(random_list_var(0,var_num), random_unary_op(), random_list_var(0,var_num))
    elif _rnd%cas_num == 1:
        return 'var%s = var%s %s var%s' % (random_list_var(0,var_num),random_list_var(0,var_num), random_judge_op(), random_list_var(0,var_num))
    elif _rnd%cas_num == 2:
        return 'var%s = var%s %s var%s ? var%s : var%s' % (random_list_var(0,var_num),random_list_var(0,var_num), random_judge_op(), random_list_var(0,var_num),random_list_var(0,var_num), random_list_var(0,var_num)) 
    elif _rnd%cas_num == 3:
        return 'var%s = var%s %s var%s' % (random_list_var(0,var_num), random_list_var(0,var_num), random_arith_op(), random_list_var(0,var_num))
'''
...
def make_func(func_name, para_num):
    ...
    _f = C.block(indent=0)
    '''
    # TODO: Local var declare
    for i in range(exp_num):
        _f.append(C.statement(make_exp(cas_num)))
    '''
    if para_num != 0:
        ret_var = random_list_var(0, para_num)
